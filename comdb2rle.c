...
{
    uint8_t *first = input->dt - 1;
    uint8_t *last = input->dt + sz - 1;
    uint8_t b = *last--;
    uint32_t dups = 0;
    ...
    while ((sz = *fld_hints) != 0) {
        ...
        default:
            if (repeats_rev(&input, sz, &tmp_r)) {
                uint32_t consume = sz * (r + 1);
                uint32_t need = space_reqd(r, sz);
                uint32_t consume_rev = sz;
                uint32_t need_rev = sz - (tmp_r + 1);
                need_rev += space_reqd(tmp_r, 1);
                if (r < 2 || (consume_rev - need_rev) > (consume - need)) {
                    // prefer rev over small repeats
                    r = tmp_r;
                    which = 'v';
                }
            }
...
int compressComdb2RLE_hints(Comdb2RLE *c, uint16_t *fld_hints)
{
    ...
    puts("");
    Comdb2RLE cc = *c;
    printf("crle %d -> %d\n", cc.insz, compressComdb2RLE(&cc) == 0 ?  cc.outsz : 0);
    printf(" lz4 %d -> %d\n", cc.insz, LZ4_compress_limitedOutput(cc.in, cc.out, cc.insz, cc.outsz));
    int rc = compressComdb2RLE_hints_int(c, fld_hints);
    printf("hint %d -> %d\n", c->insz, rc == 0 ? c->outsz : 0);
    return rc;
}
#endif
